#' Landings LFD raising
#'
#' Prepare cleaned RCG sampling and landings templates and raise landings
#' length-frequency distributions (LFDs).
#'
#' This function is the main landings raising step in the stat4RDBFIS workflow.
#' It builds the stock and stratum inputs, checks observed against estimated
#' subsample weights, applies the selected raising method, and writes the
#' raised landings LFD outputs.
#'
...
#'   include `"trip_h"`, `"trip_cc"`, `"vigneau_mahevas"`, `"method_grc"`,
#'   `"method_ita2"`, `"method_ita3"`, `"method_esp"`, `"method_cyp"`, and
#'   `"method_fra"`.
...
#'   landings LFDs are written. Default is `"output"`.
#'
#' @return A data.frame with raised landings LFDs in long format. The function
#'   also writes MED&BS landings-template outputs and subsample-weight
#'   diagnostics to `out_dir`.
#'
#' @author stat4RDBFIS Core Team
#' @export landings_rdbfis
...
# version: 0.0.8 (output MED&BS landings template)
# date: March 2026

landings_rdbfis <- function(sampling_df, landings_df,
                            SP = NULL, GSA = NULL, YEAR = NULL, method = NULL, sample_types=c('S', 'D', 'M'), use_mat=FALSE,
                            out_dir = "output") {

  # 1) check dependencies
  # Packages must be loaded before calling landings_rdbfis() - see tutorial step 0.
  ...
  package_env <- environment(landings_rdbfis)
  ...
  # We only check for the fields that are strictly necessary for the preparation of lfd_base and landings_totals.
  required_landings <- c(
  ...
  # 10) Restrict Sampling to LFD Landings scope
  sampling_df <- subset(sampling_df, catch_category == "Lan")

  # 10a) Estimate number of samples per stratum
  nsample      <- sampling_df %>%
  ...
  #     RCG template specification: length_class is ALWAYS in mm.
  #     length_code is NOT the unit - it is the measurement precision.
  #     Wrong units (e.g. cm instead of mm) are a data quality issue detected
  #     by estimate_sub_sample_w() via the observed/estimated weight ratio.

  crustaceans <- c('DPS', 'ARS', 'ARA', 'NEP', 'TGS', 'MTS')
  idx <- !(lfd_base$species %in% crustaceans)
  lfd_base$lc[idx] <- floor(lfd_base$lc[idx]/10) * 10

  # 12) Aggregate official landings at MS, area, species, year, quarter, metier level
  ...
  res <- list(
    stocks          = stocks,
    lfd_base        = lfd_base,
    landings_totals = landings_totals
  )
  ...
  cat("\nLFD Landings input preparation completed for all stocks.\n")
  ...
  indl <- unique(res$landings_totals[ , c('stock', 'year')])
  ...
  raise_VM <- function(res) {
    ...
    result_list <- lapply(stocks, function(s) {
      ...
      land_stock <- res$landings_totals[res$landings_totals$stock == s, ]
  ...
  raise_ESP <- function(res) {
    ...
    result_list <- lapply(stocks, function(s) {
      ...
      method_esp(lfd_stock, land_stock)  # call stock-level raising function
    })
  ...
  raise_ITA <- function(res) {
    ...
    for (i in 1: length(method)){
      ...
      res_split[[i]]$landings_totals <- subset(res$landings_totals, method==method[i])
    }
  ...
  # merge with landings totals to get also the landings of strata for which no lfd was sampled
  medbs_out <- merge(raised_lfd, landings_totals, by=c("flag_country", "year", "quarter", "area",  "species", "metier"), all.x=T, all.y=T)
  ...
  colnames(medbs_out) <-  c("country", "year", "quarter", "area",  "species", "gear",
                               "lc", "landings", "N_final", "method", "sample_size")

  # get landings in tons, numbers in 1000 individuals and length bins in cm
  medbs_out$lc <- medbs_out$lc/10
  medbs_out$landings <- medbs_out$landings/1000
  ...
  readr::write_csv(medbs_out, file.path(out_dir, paste0("landings_", file_suffix, "-" , method, ".csv")))
