...
discard_ratio_rdbfis <- function(
    sampling_df,
    out_dir = file.path("discard_ratio"),
    MS = NULL,
    GSA = NULL,
    YEAR = NULL,
    SP = NULL,
  ...
  required_cols <- c(
    ...
    "weight", "aggregation_level","number_at_length", "length_class",
    "station_code", "trip_code"
  )
  ...
  dat <- dat %>%
    mutate(
      ...
      length_class = suppressWarnings(as.integer(length_class)),
  ...
  process_one_level <- function(dat) {
    ...
    dat_filt <- dat_filt %>%
      transmute(
        ...
        metier,
        species,
        psu = paste0(trip_code, "-", station_code),
        catch_category,
        weight,
        length_class,
        number_at_length
    ...
    results_list <- lapply(groups, function(df) {
      ...
      out_one <- cbind(
        ...
        data.frame(
          ...
          landings = est$landings,
          total = est$total,
          discard_ratio_total = round(est$discard_ratio_total, 6),
          discard_ratio_total_lwr = round(ci_total[1],6),
          discard_ratio_total_upr = round(ci_total[2],6),
          discard_ratio_landings = round(est$discard_ratio_landings,6),
          discard_ratio_landings_lwr = round(ci_land[1],6),
          discard_ratio_landings_upr = round(ci_land[2],6),
          stringsAsFactors = FALSE
        )
      )
