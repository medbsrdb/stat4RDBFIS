#' Maturity at length outputs from cleaned sampling data
#'
#' Applies the maturity-at-length workflow to the cleaned sampling template and
...
#' @param LC optional length-class setting passed to `ml()`.
#' @param out_dir character. Output directory where csv files are written.
#'
#' @return A nested list of maturity-at-length results by stock and year.
#'
...
#' }
#' @export
ml_rdbfis <- function(sampling_df, SP = NULL, GSA = NULL, YEAR = NULL, Imm=c(0,1), LC=NULL, out_dir = "maturity") {
  ...
  for (s in st) {
    ...
    yr_result <- ml(temp1_, Species, MS, Area, styear, endyear, Imm, LC)
    yr_out[[yr]] <- yr_result
  ...
  attr(ml_out, "fit_warnings") <- fit_warning_log
  if (length(fit_warning_log) > 0) {
    message(
      "ml_rdbfis: glm fit warnings were recorded for ",
  ...
  per_sex_all_raw <- do.call(
    rbind,
    lapply(names(ml_out), function(stock_name) {
      stock <- ml_out[[stock_name]]
      do.call(
        rbind,
        lapply(names(stock), function(year_name) {
          ...
          df$year  <- year_name
          df}))}))
  per_sex_all_raw=per_sex_all_raw[,1:11]
    ...
    lapply(names(ml_out), function(stock_name) {
      ...
      do.call(
        ...
        lapply(names(stock), function(year_name) {
          ...
          df$year  <- year_name
          df}))}))
  common_sex_all_raw=common_sex_all_raw[,1:11]
    ...
    lapply(names(ml_out), function(stock_name) {
      ...
      do.call(
        ...
        lapply(names(stock), function(year_name) {
          ...
          df$year  <- year_name
          df}))}))
  per_sex_all_model=per_sex_all_model[,1:11]
    ...
    lapply(names(ml_out), function(stock_name) {
      ...
      do.call(
        ...
        lapply(names(stock), function(year_name) {
          ...
          df$year  <- year_name
          df}))}))
  common_sex_all_model=common_sex_all_model[,1:11]
  ...
  write.csv(common_sex_all_raw, file.path(out_dir, paste0("ml_common_sex_all_raw_", file_suffix, ".csv")), row.names=F)
  write.csv(per_sex_all_raw, file.path(out_dir, paste0("ml_per_sex_all_raw_", file_suffix, ".csv")), row.names=F)
  write.csv(per_sex_all_model, file.path(out_dir, paste0("ml_per_sex_all_model_", file_suffix, ".csv")), row.names=F)
  write.csv(common_sex_all_model, file.path(out_dir, paste0("ml_common_sex_all_model_", file_suffix, ".csv")), row.names=F)
